  ...
  return (
    <>
      ...
      <section style={{ background: '#05050D', padding: '160px 24px 100px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: '-25%', left: '50%', transform: 'translateX(-50%)', width: 700, height: 500, borderRadius: '50%', background: 'radial-gradient(circle,rgba(123,47,190,0.28) 0%,transparent 65%)', filter: 'blur(80px)', pointerEvents: 'none' }} />
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24, alignItems: 'start' }} className="pricing-grid">
            {tiers.map(({ name, price, tagline, desc, features, cta, featured, badge }) => (
              ...
              >
                {badge && (
                  <div style={{ position: 'absolute', top: -14, left: '50%', transform: 'translateX(-50%)', background: 'linear-gradient(135deg,#7B2FBE,#06B6D4)', borderRadius: 100, padding: '5px 18px', fontFamily: 'var(--font-jetbrains),monospace', fontSize: '0.65rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: '#fff', whiteSpace: 'nowrap' }}>
                    {badge}
                  </div>
                )}

                {/* Plan name */}
                <p style={{ fontFamily: 'var(--font-jetbrains),monospace', fontSize: '0.7rem', letterSpacing: '0.14em', textTransform: 'uppercase', color: featured ? '#06B6D4' : '#7B53CC', marginBottom: 12 }}>{name}</p>

                {/* Price */}
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, marginBottom: 8 }}>
                  <span style={{ fontFamily: 'var(--font-dm-sans),sans-serif', fontSize: '1rem', color: '#8B8BA8', marginBottom: 10 }}>$</span>
                  <span style={{ fontFamily: 'var(--font-syne),sans-serif', fontWeight: 800, fontSize: 'clamp(2.8rem,4vw,3.5rem)', lineHeight: 1, letterSpacing: '-0.04em', color: '#F0F0FF' }}>{price}</span>
                  <span style={{ fontFamily: 'var(--font-jetbrains),monospace', fontSize: '0.75rem', color: '#8B8BA8', marginBottom: 10 }}>/mo</span>
                </div>

                {/* Tagline */}
                <p style={{ fontFamily: 'var(--font-jetbrains),monospace', fontSize: '0.7rem', letterSpacing: '0.06em', color: '#8B8BA8', marginBottom: 16, lineHeight: 1.6 }}>{tagline}</p>

                <p style={{ color: '#8B8BA8', fontSize: '0.9rem', lineHeight: 1.72, marginBottom: 28 }}>{desc}</p>
                ...
                {/* Divider */}
                <div style={{ height: 1, background: 'rgba(123,83,204,0.2)', marginBottom: 28 }} />
                  ...
                  {features.map(f => (
                    <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <svg width="18" height="18" viewBox="0 0 18 18" fill="none" aria-hidden="true" style={{ flexShrink: 0 }}>
                        <circle cx="9" cy="9" r="8" stroke={featured ? '#06B6D4' : '#7B53CC'} strokeWidth="1.3"/>
                        <path d="M5.5 9l2.5 2.5 4.5-5" stroke={featured ? '#06B6D4' : '#8B5CF6'} strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/>
                      </svg>
                      <span style={{ color: f.startsWith('Everything') ? '#7B53CC' : '#F0F0FF', fontSize: '0.9rem', fontStyle: f.startsWith('Everything') ? 'italic' : 'normal' }}>{f}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* ── BILLING NOTE ── */}
      <section style={{ background: '#05050D', padding: '0 24px 100px' }}>
        <div style={{ maxWidth: 600, margin: '0 auto', textAlign: 'center' }}>
          <p style={{ fontFamily: 'var(--font-jetbrains),monospace', fontSize: '0.72rem', letterSpacing: '0.1em', textTransform: 'uppercase', color: '#8B8BA8' }}>
      ...
      <section style={{ background: '#0A0A1A', padding: '100px 24px' }}>
        <div style={{ maxWidth: 760, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 60 }}>
            ...
            <h2 style={{ fontFamily: 'var(--font-syne),sans-serif', fontWeight: 700, fontSize: 'clamp(1.8rem,4vw,2.8rem)', letterSpacing: '-0.03em', color: '#F0F0FF' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            {faqs.map(({ q, a }) => (
              <div key={q} className="glass" style={{ padding: '28px 32px' }}>
                <p style={{ fontFamily: 'var(--font-syne),sans-serif', fontWeight: 700, fontSize: '1rem', color: '#F0F0FF', marginBottom: 10 }}>{q}</p>
                <p style={{ color: '#8B8BA8', fontSize: '0.9375rem', lineHeight: 1.72 }}>{a}</p>
              </div>
            ))}
          </div>
        </div>
